...
def search_max_documents(kwargs):
    ...
    while True:
        logger.info(
            f"Searching page {current_page}, currently at {(len(results['documents']))} documents"
        )
        search_args = {**kwargs, "limit": 1000, "current_page": current_page}
        page_results = search(**search_args)
        ...
        all_documents_loaded = (
            len(results["documents"]) >= results["meta"]["total_documents"]
        )
...
def search(
    ...
    for result in data["results"]:
        row = {"score": result["_meta"]["score"] or 0}
    ...
    return {
        "meta": {"total_documents": data["meta"]["page"]["total_results"]},
        "documents": results,
    }
